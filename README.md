...
> **Java 25 + LibGDX で書く Desktop 向け戦術ローグライト**

死を繰り返して強くなる、3〜5分の最小ループ。カード・AP・罠・ソウルツリーを 1 つのループへ統合した 2D ピクセルアート作品。

---

## What is this?

部内ハッカソンの定番フォーマット（JS + Web）を外し、**Java + DDD で戦闘規則を純粋なドメイン層へ閉じ込める** ことに挑戦する作品。
LibGDX の LWJGL3 backend を採用し、現行版は Desktop 向けに配布する。
...
現行版の配布対象は **Desktop のみ**。表記は実際に検証した範囲に揃える。

| 対象 | 状態 |
|---|---|
| **Windows** | 実行検証済み |
| **macOS / Linux** | 未検証。OS マトリクス CI 導入後に「ビルド検証済み」と表記する (RELEASE-01) |
| **Android / 端末間セーブ同期** | **対象外**。Android backend・タッチ導線・同期機構のいずれも未実装 |
| Web / iOS | スコープ外 |

> Android を将来の中心に据える場合は、共有 core の言語水準、モジュール分離、タッチ E2E、セーブ同期方式を含む独立した計画が必要になる。現行ロードマップには含めない。
...
**ライセンス指定なし (オーナー保有)**。第三者による利用・再配布は要相談。
