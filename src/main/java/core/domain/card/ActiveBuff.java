 ...
 *
 * <p>{@link CardEffect.Buff} がカードのデータ定義 (kind / amount / durationTurns) なのに対し、本 record は戦闘中に Player に
 * 付与されたあとの「残ターン数」を含む動的状態。{@code PlayerStatuses.activeBuffs} に保持される (ADR-27)。
 *
 * <p>ターン経過: {@link #decrementedTurn()} で残ターン -1、0 になったら除去 ({@link #isExpired()})。
 * 重複 {@link CardEffect.BuffKind} は加算合成ではなく上書き (KISS、ADR-22 罠の上書きパターン踏襲、ADR-27)。
 */
