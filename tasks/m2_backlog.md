...
- **完了 (M2 Wave 15)**: 仕様整合性監査 + 不整合是正 5 件。P0 致命バグ 4 件 (W15-α、commit 9337d14): #11 セーブロード Gold/runSoul 消失 / #7 CAUTIOUS AI 無限ウェイト / #10 SPEED_UP 持続 1 ターン論理消滅 / #17 デッキ薄スタート 5 ドロー山札枯渇。SkillSlot 完全廃止 #3 (W15-β、commit 29afc4c)。Rest HP 30% 制限 #2 (W15-γ、commit 47dc4da)
- **完了 (M2 Wave 16)**: SpecificationIssues.md 残 5 件対応 (#8 のみ Wave 17 送り)。チュートリアル文言充実 #5 + HUD スキル枠完全削除 #6 (W16-α、commit 09104ba)。壊れる壁配置を対向 FLOOR 縛りで真のショートカット優先化 #1 (W16-β、commit 1ad954c)。ShopEquipment ラン中即時反映 #9 + maxHp 増加分の currentHp 同期 (W16-γ、commit 76e3741)
- **完了 (M2 Wave 17)**: #8 UiTheme ライト/ダーク 2 トグル化 (SpecificationIssues.md 最終 1 件)。ThemeMode enum + Settings.themeMode 拡張 + i18n + graceful migration (W17-α、commit 4741ab8)、UiThemeResolver 装備依存廃止 + UiTheme.light() 追加 + DddGame.activeUiTheme で Settings 直参照 (W17-β、commit 4d6c265)、SettingsScreen にテーマ切替 UI 追加でリアルタイム反映 + m2_backlog 反映 (W17-γ、commit 予定)。CTO 3 チェックポイント遵守: #1 Jackson 複数コンストラクタ罠回避 (record 単一コンストラクタ)、#2 フォント色ハードコード grep でステルステキストバグ防衛 (light() 濃灰テキストで視覚検出可)、#3 リアルタイム反映 (DddGame.activeUiTheme 毎フレーム評価で UI 即明暗反転)
- **M2 送り (Wave 18+ 候補)**:
  - **#4 ソウルツリー描画修正**: ユーザー判断「気にしない」でスコープ外、将来必要なら Wave 18+ で実機再現 → コード診断 (UI 描画 vs tree.json データ) → CTO チェックポイント (データと UI 分離原則) に則り修正
  ...
  - **Wave 15 で Wave 16+ 送りした 9 件** (SpecificationIssues.md / 追加監査由来):
    - **#9 ShopEquipment ラン中即時反映 (CTO #4)**: PlayerStatuses.equipment 差し替え + grantedCards を drawPile にシャッフル追加。実装規模 M、Wave 16+ で着手
    - **#1 壊れる壁の生成位置改善** (部屋間ショートカット): DungeonGenerator 構造把握要、規模 M
    - **#8 UiTheme 簡素化** (装備依存 → ライト/ダーク 2 トグル): 設定画面追加要、規模 M
    - **#4 ソウルツリーの「いっこ飛ばし」描画修正**: SoulTreeScreen リファクタ、規模 M
    - **#5 / #16 チュートリアル UI 確認** (装備画面経由スキップ問題、初回フラグ動線): TutorialOverlay 実装確認 + 表示条件強化
    - **#13 敵予告 AP 実行時整合** (UI 未実装ならスコープ外、Bestiary 次行動予告 UI 着手時に対応)
    - **#18 AP 使い切り型の仕様書 breaking change 明文化** (ドキュメント整理)
    - **#19 探索 vs 戦闘モードの UI 区別**
    - **#20 層 1 ノード選択肢追加 + 金貨詰み対策** (バランス調整)
  ...
  - **Bestiary 次行動の点線予告 UI** (Wave 13 で敵 AI の視界・状態が入ったため、HUD で「視認中」「探索中」アイコン表示も実装余地)
