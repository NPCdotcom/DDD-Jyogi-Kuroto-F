...
- **完了 (M2 Wave 16)**: SpecificationIssues.md 残 5 件対応 (#8 のみ Wave 17 送り)。チュートリアル文言充実 #5 + HUD スキル枠完全削除 #6 + ソウルツリー描画 #4 はユーザー実機確認待ち (W16-α、commit 09104ba)。壊れる壁配置を対向 FLOOR 縛りで真のショートカット優先化 #1 (W16-β、commit 1ad954c)。ShopEquipment ラン中即時反映 #9 (W16-γ、commit 予定): PlayerStatuses.equipment 差し替え + grantedCards を drawPile にシャッフル追加 + maxHp 増加分の currentHp 同期 (CTO #3) で「最大 HP だけ伸びて傷ついて見える」UX 違和感を解消
- **M2 送り (Wave 17+ 候補)**:
  - **#8 UiTheme ライト/ダーク 2 トグル化** (SpecificationIssues.md 残 1 件、最優先): Settings.themeMode (LIGHT/DARK) field + UiThemeResolver 装備依存廃止 + SettingsScreen テーマ切替 UI + 全 UiThemeResolver 呼出箇所更新。規模 M、Wave 17 で着手
  - **#4 ソウルツリー描画修正**: 実機で「いっこ飛ばし」現象を再現 → コード診断 (UI 描画 vs tree.json データ) → CTO チェックポイント #1 (データと UI 分離原則) に則り修正
