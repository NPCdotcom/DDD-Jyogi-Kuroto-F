...
- **完了 (M2 Wave 15)**: 仕様整合性監査 + 不整合是正 (新機能スコープ外、SpecificationIssues.md 11 件 + 追加 9 件監査)。P0 致命バグ 4 件 (W15-α、commit 9337d14): #11 セーブロード Gold/runSoul 消失 (SaveData v3 + restoreLayer 復元) / #7 CAUTIOUS AI 無限ウェイト (距離 2-3 + 遠距離スキル不在で AGGRESSIVE フォールバック) / #10 SPEED_UP 持続 1 ターン論理消滅 (Buff/AP リフィル順序 swap) / #17 デッキ薄スタート 5 ドロー山札枯渇 (CardPileState.drawToHandSize 3 段防衛線で 5 枚補充)。SkillSlot 完全廃止 (W15-β、commit 29afc4c): KISS 案で Player.skillSlot を SkillSlot.empty(4) 化 + PlayerInputs F1-F4 削除 + HudRenderer.drawSkillSlots 呼出削除 + bindSkillSlotSizeSupplier 廃止 (§15-3「デッキ・カード一本化」整合、敵側スキル経路は無修正)。Rest HP 30% 制限 (W15-γ、commit 予定、#2 バランス是正)
- **M2 送り (Wave 16+ 候補)**:
