...
> **重要な但し書き (2026-08-12、敵対的検証で判明)**
>
> 以下の SAVE-01 / 02A / 02B / 03A は **型と変換器を用意した段階であり、本番経路へ接続されていない**。
> `saveProfile` / `loadProfile` / `saveCheckpoint` / `LegacySaveMigrator` / `RunCheckpointMapper` /
> `RunInventory` の本番呼出は **0 件**。`PersistenceServices` は migrator を呼ばず、`TitleScreen` の
> 「つづき」は今も旧 `save.json` の `exists()` を見ている。
>
> したがって **レビュー P0-1 (ラン終了で進捗が消える / 終了済みランを再開できる) は実際のゲームでは
> まだ直っていない**。接続は SAVE-03B の担当で、そこで 4 タスクぶんの不整合が同時に露出しうる。
> 「✅」は各タスクの受入条件に対するものであり、P0-1 の解消を意味しない。

#### SAVE-01 ✅ 完了 (2026-08-12) — 受入条件 3 件が未達
...
#### SAVE-03A ✅ 完了 (2026-08-12) — 受入条件 3 件を他タスクへ移設

`RunId` を新設して `RunSession` に持たせ、`RunCheckpointMapper` で Checkpoint ↔ ラン状態を変換する。
`toCheckpoint` は保護数が保護枠を超える場合に呼出元へ分かる例外を投げる (層境界セーブ中の
オートセーブ落ちを防ぐ)。

#### 敵対的検証で判明した未達の受入条件

各タスクの受入条件のうち、**実装していないもの**を正直に記録する。「実装したことにする」は不可。

| タスク | 未達の受入条件 | 対応 |
|---|---|---|
| SAVE-01 | 原子的置換が未対応の場合に「再読込検証した後だけ `.bak` を削除」 | ✅ 修正済 (期待バイト列と突合) |
| SAVE-02A | カタログ外 ID をログへ残し、既知の装備と進捗を巻き添えで失わない | ❌ カタログ照合が未実装。別 Issue |
| SAVE-02B | 再起動後に**不足ファイルだけ**を復元する | ❌ journal の `profileWritten`/`checkpointWritten` は書かれるだけで読まれず、実際は毎回全部書き直す。決定的再計算なので結果は正しいが文言は未達。フラグを使うか条件を書き換えるか要判断 |
| SAVE-02B | Profile と RunCheckpoint を再読込して**移行 ID と値**を検証 | ❌ 移行 ID が Profile にも Checkpoint にも保存されておらず照合不能。値のみ検証している |
| SAVE-01 / 02B | 計画が対象に挙げた `SaveManagerTest` 拡張、`legacy-save-fixtures.json`、`PersistenceServices` 接続 | ❌ 未着手 |
| SAVE-03A | ラン開始時に Profile の保有 Soul を 0 へ移さない / 保有 Soul はツリー購入だけに使う | → **SETTLE-01 へ移設**。精算未実装のまま外すとラン中 Soul の行き先が無くなるため |
| SAVE-03A | Checkpoint から復元した RunSession を再保存してもランIDと一時所持品を維持 | → **SAVE-03B へ移設**。RunSession を復元する経路が SAVE-03B の担当 |

**SETTLE-01 着手時の順序警告**: 先に `DddGame.preserveSoulFromRun()` を「上書き」から「Profile 保有は
そのまま、ラン中 Soul のみ加算」へ変え、**その後で** `startNewRun()` の注入と 0 化を削る。逆順にすると
ラン終了時に Profile 保有分が `player.soul()` で上書きされ、ソウルツリー用の貯金が全損する。

#### 第2バッチの実測 (SAVE-03A まで)
...
#### SAVE-03B ✅ 完了 (2026-08-13) — 本番接続済、P0-1 解消

`RunLifecycle` (LibGDX 非依存) を新設し、読込・書込の両側を新形式へ接続した。

- 再開条件は「Checkpoint が存在し `runId` が Profile の `activeRunId` と一致」
- 書込順序: 層境界は Checkpoint → Profile、終了時は Profile → Checkpoint 削除。どちらも途中で落ちたら「再開不可」側へ倒れる
- `onRunEnded()` が恒久進捗を保存する (**P0-1 (a) の本体**)
- 進行中ランがある状態の新規開始は `ConfirmationDialog` で確認し、放棄は §15-9 どおり死亡と同じ規則で精算する
- `create()` で `profile.json` から `progress` を復元する
- `SaveLifecycleContractTest` 14 件 + `ProfileDataMapperTest` 6 件

#### 第2バッチで作り込み、自分で塞いだ欠陥 (記録)

自動テストが緑のまま素通りした P0 級が 4 件あった。いずれも**本番の呼出元を通るテストが無かった**ことが共通原因。

| # | 欠陥 | 発見 |
|---|---|---|
| 1 | 読込だけ新形式へ替え書込が旧形式のまま (2 層目以降の進捗が消える) | 自己確認 |
| 2 | 層境界セーブで `profile.soulTotal` が 0 に潰れ、中断離脱で持越しソウル全損 | 敵対的検証 |
| 3 | 放棄確認が「死亡と同じ扱い」と表示するのに精算していない | 自己確認 |
| 4 | 起動時に `progress` を復元せず、次のラン開始で `profile.json` を全消去 | 敵対的検証 |

**教訓**: `SaveLifecycleContractTest` は手書きの正しい `ProfileData` を `RunLifecycle` へ渡しており、「呼出側が正しく組み立てられる」ことを前提にしていた。壊れていたのはその前提そのもの。緑が保証するのは順序規則だけで、配線は保証しない。
...
| 項目 | 完了条件 | 優先度 |
|---|---|---|
| ~~**手動通し確認**~~ | ✅ 2026-08-13 実施。保存・再開の通しで**問題なし**。所見 5 件は下表へ | 完了 |
| `ConfirmationDialog` のマウス対応 | クリックで開いたダイアログをクリックで閉じられる | P2 |
| カタログ外 ID の保全 | `toPlayerProgress` → 書込で未知 ID が消えない、または消えたことを通知する | P2 |
| `.legacy-backup` の世代 | 2 回目の移行で最初の原本が失われない | P2 |
| SAVE-02A の未達 | カタログ外 ID をログへ残し既知の進捗を巻き添えにしない | P1 |
| `legacy-save-fixtures.json` | 実 JSON からの移行試験がある | P2 |

### 手動通し確認の結果 (2026-08-13)

保存・再開の通し (保存先削除 → 起動 → 1 層クリア → 終了 → 起動 → つづき) は**問題なし**。第2バッチで直した P0-1 は実機で解消を確認できた。

以下は通しプレイ中の所見 5 件。**うち 2 件は自動テスト 875 件と敵対的検証 8 回をすり抜けた新規指摘**で、実際に遊ばないと出ない種類だった。

| # | 所見 | 素性 | 完了条件 | 優先度 | 依存 |
|---|---|---|---|---|---|
| 1 | 敵ターン中の表示が小さく、待たされている状況が分かりにくい | **新規**。レビュー §9 は「層3 で 8 体 × 0.1 秒 = 2.4 秒の入力不能」を仮説として挙げていたが、その間の視認性は誰も指摘していなかった | 敵ターン開始時に画面中央へ大きく表示し、終了で消える。2.4 秒の入力不能が「待たされている」と分かる | P1 | なし |
| 2 | 壊れる壁が有用でない位置に置かれる | **新規**。`placeBreakableWalls` は Wave 16 で「対向 2 方向が FLOOR = ショートカット壁」優先の改善済みだが、効いていない疑い。候補が 2 個未満だと旧仕様の fallback へ落ちる実装 | ショートカット候補の抽出条件を見直し、fallback へ落ちる頻度を計測して減らす | P2 | なし |
| 3 | 装備がまだ全開放 | **既知**。レビュー P0-2 そのもの (`EquipmentScreen.java:68` が `equipmentCatalog().all()`)。EQUIP-01/02 未着手なので直っていなくて正常 | EQUIP-01/02 の完了条件に従う | P0 | EQUIP-01 |
| 4 | ソウルツリーの開放が一個飛ばしに見える | **既知**。`FeatureCatalog.md:511` の「#4 ソウルツリー描画 (既知 UI 不具合): 枝線/座標の不整合」。ユーザー確認により**解放判定ではなく描画位置のズレ**と判断 | ノードと枝線の座標が一致し、解放済み/未解放の対応が見た目で正しく読める | **P1 (P2 から格上げ)** | なし |
| 5 | スケルトンの AI 挙動がおかしい | **仕様どおり**。スケルトン = `SWIFT_SLIME` で、これだけ `CAUTIOUS` (距離 2-3 を保つ kite 型、隣接されたら離れる)。他は全て `AGGRESSIVE` | 挙動は変えない。レビュー §7.7 のとおり**名称・見た目・挙動の三者が揃うよう敵 ID と表示名を改名**する | P2 | なし |

**#4 を格上げした理由**: 「優先度低の描画不整合」として記録されていたが、実際に遊ぶと**解放判定のバグに見える**。プレイヤーが「ノードが飛ばされた」と誤解する状態は、純粋な見た目のズレより実害が大きい。

**#5 の注意**: 挙動自体は意図的だが、「はやスライム」という名前でスケルトンの見た目で逃げ回るという三重のズレがあり、プレイヤーには不具合に見える。画像側ではなく**名前側を実像へ寄せる**のがレビューの結論 (§7.7)。

次バッチは EQUIP-01 (所有装備の不変条件) から。上表 #3 は EQUIP-01/02 で解消する。
